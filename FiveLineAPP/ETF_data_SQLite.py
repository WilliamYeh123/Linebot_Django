...
def get_sql():
    ...
    #conn = newSystem.Data.SQLite.SQLiteConnection("Data Source="+Application.StartupPath+//.ETF_list.db)
    cursor = conn.cursor()
    sql = "select volume from ETF_data where Date = ? and Number = ?"
    cursor.execute(sql, ('2018-12-11', '0050'))
...
def basic_information(msg):
    ...
    conn = sqlite3.connect('/home/awinlab/Documents/yeh/LineBot/FiveLineAPP/ETF_list.db')
    cursor = conn.cursor()
    sql = f"select * from ETF_data where Number = '{msg}'"
    cursor.execute(sql)
    rows = cursor.fetchall()
    #print(rows)
    newList = list(reversed(rows))
    content = (f'Date: {newList[0][0]}\nNumber: {newList[0][1]}\nOpen: {newList[0][2]}\nHigh: {newList[0][3]}\nLow: {newList[0][4]}\nClose: {newList[0][5]}\nVolume: {newList[0][6]}')
